...
class TestHTTPSService(VyOSUnitTestSHIM.TestCase):
    ...
    def test_api_auth(self):
        ...
        headers = {}
        r = request('POST', url, verify=False, headers=headers, data=payload, timeout=60)
        # Must get HTTP code 200 on success
        self.assertEqual(r.status_code, 200)

        payload_invalid = {'data': '{"op": "showConfig", "path": []}', 'key': 'invalid'}
        r = request('POST', url, verify=False, headers=headers, data=payload_invalid, timeout=60)
        # Must get HTTP code 401 on invalid key (Unauthorized)
        self.assertEqual(r.status_code, 401)

        payload_no_key = {'data': '{"op": "showConfig", "path": []}'}
        r = request('POST', url, verify=False, headers=headers, data=payload_no_key, timeout=60)
        ...
        r = request('POST', graphql_url, verify=False, headers=headers, json={'query': query_valid_key}, timeout=60)
        ...
        r = request('POST', graphql_url, verify=False, headers=headers, json={'query': query_invalid_key}, timeout=60)
        success = r.json()['data']['SystemStatus']['success']
        ...
        r = request('POST', graphql_url, verify=False, headers=headers, json={'query': query_no_key}, timeout=60)
        success = r.json()['data']['SystemStatus']['success']
        self.assertFalse(success)
        ...
        """
        r = request('POST', graphql_url, verify=False, headers=headers, json={'query': mutation}, timeout=60)
        ...
        r = request('POST', graphql_url, verify=False, headers=headers, json={'query': query}, timeout=60)
    ...
    def test_api_config_file_load_http(self):
        ...
        r = request('POST', url_config, verify=False, headers=headers, data=payload, timeout=60)
        self.assertEqual(r.status_code, 200)
